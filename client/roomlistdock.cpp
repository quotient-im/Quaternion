...
RoomListDock::RoomListDock(MainWindow* parent) : QDockWidget("Rooms", parent)
{
    ...
    connect(view, &QTreeView::activated, this,
            &RoomListDock::rowSelected); // See #608
    ...
    connect(model, &RoomListModel::rowsInserted, this,
            &RoomListDock::refreshTitle);
    connect(model, &RoomListModel::rowsRemoved, this,
            &RoomListDock::refreshTitle);
    ...
    connect(model, &RoomListModel::restoreCurrentSelection, this, [this] {
        const auto& idx = model->indexOf(selectedGroupCache, selectedRoomCache);
        ...
        if (!groupState.isValid()) {
            if (groupKey.startsWith(RoomGroup::SystemPrefix)) {
                const auto legacyKey =
                    RoomGroup::LegacyPrefix
                    + groupKey.mid(RoomGroup::SystemPrefix.size());
    ...
    addTagsAction = roomContextMenu->addAction(QIcon::fromTheme("tag-new"),
                                               tr("Add tags..."), this,
    ...
    roomContextMenu->addSeparator();
    joinAction = roomContextMenu->addAction(
        QIcon::fromTheme("irc-join-channel"), tr("Join room"), this, [this] {
            if (auto room = getSelectedRoom()) {
    ...
    roomContextMenu->addSeparator();
    forgetAction = roomContextMenu->addAction(
        QIcon::fromTheme("irc-remove-operator"), tr("Forget room"), this, [this] {
            if (auto room = getSelectedRoom()) {
                Q_ASSERT(room->connection());
