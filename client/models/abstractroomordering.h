...
struct RoomGroup {
    ...
    bool operator==(const RoomGroup& other) const { return key == other.key; }
    bool operator!=(const RoomGroup& other) const { return !(*this == other); }
    ...
    friend bool operator==(const QVariant& otherCaption, const RoomGroup& group)
    {
    ...
    static inline const auto LegacyPrefix =
        QStringLiteral("org.qmatrixclient.");
};
...
public: // Overridables
    ...
    virtual bool roomLessThan(const QVariant& group, const Room* r1,
                              const Room* r2) const = 0;
...
public:
    ...
    using roomLessThan_closure_t = std::function<bool(const Room*, const Room*)>;
