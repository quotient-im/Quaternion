...
{
    return isValidRoomIndex(idx) ? static_cast<QuaternionRoom*>(
...
void RoomListModel::connectRoomSignals(Room* room)
{
    ...
    connect(room, &Room::avatarChanged, this,
            [this, room] { refresh(room, { Qt::DecorationRole }); });
}
...
QVariant RoomListModel::data(const QModelIndex& index, int role) const
{
    ...
    if (isValidGroupIndex(index)) {
        ...
        if (role == Qt::BackgroundRole)
            return QGuiApplication::palette().brush(QPalette::Active,
                                                    QPalette::Button);
        return {};
    }
    ...
    switch (role) {
    case Qt::DisplayRole: {
        const auto prefix = room->isUnstable() ? QStringLiteral("(!)")
                                               : QString();
        const auto unreadCount = room->unreadCount();
        const auto postfix =
            unreadCount == -1 ? QString()
    ...
    case Qt::ToolTipRole: {
        QString result =
            ...
            % tr("Joined: %Ln", "The number of joined members",
                 room->joinedCount());
        if (room->invitedCount() > 0)
            result += "<br>"
                      % tr("Invited: %Ln", "The number of invited users",
                           room->invitedCount());
        ...
        if (!directChatUsers.isEmpty()) {
            ...
            result +=
                "<br>" % tr("Direct chat with %1").arg(userNames.join(','));
        }
        ...
        if (room->isUnstable()) {
            ...
            if (room->canSwitchVersions())
                result += ' '
                          % tr("Consider upgrading to a stable version"
                               " (use room settings for that)");
        }

        auto unreadCount = room->unreadCount();
        if (unreadCount >= 0) {
            const auto unreadLine = room->readMarker() == room->timelineEdge()
        ...
        auto asUser =
            m_connections.size() < 2
                ? QString()
                : ' '
                      + tr("as %1", "as <user account> (disambiguates entries "
                                    "in the room list)")
    ...
    case JoinStateRole:
        ...
        return toCString(room->joinState()); // FIXME: better make the enum
                                             // QVariant-convertible (only
                                             // possible from Qt 5.8, see
                                             // Q_ENUM_NS)
    case ObjectRole:
...
int RoomListModel::columnCount(const QModelIndex&) const { return 1; }
