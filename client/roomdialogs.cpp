    ...
    , mainFormLayout(addLayout<QFormLayout>())
{
    ...
    topic->setSizeAdjustPolicy(QAbstractScrollArea::AdjustToContentsOnFirstShow);
    ...
    if (!room) // TODO: Support this in RoomSettingsDialog as well
    {
        ...
        //        formLayout->addRow(guestCanJoin); // TODO:
        //        quotient-im/libQuotient#36
    }
}
                                           ...
                                           Connection* account)
{
    ...
    if (account->loadingCapabilities()) {
        selector->addItem(tr("(loading)",
                             "Loading room versions from the server"),
                          QString());
    ...
    for (const auto& v: versions) {
        ...
        const auto postfix =
            isDefault
                ? tr("default", "Default room version")
                : v.isStable() ? tr("stable", "Stable room version") : v.status;
    ...
    , tagsList(new QListWidget)
{
    ...
    if (room->isUnstable())
        versionBox->addWidget(
            new QLabel(tr("This version is unstable! Consider upgrading.")), 1,
            0);
    if (room->canSwitchVersions()) {
        ...
        connect(changeActionButton, &QAbstractButton::clicked, this, [=] {
            Dialog chooseVersionDlg(tr("Choose new room version"), this,
                                    NoStatusLine,
                                    tr("Upgrade", "Upgrade a room version"),
                                    NoExtraButtons);
    ...
    //        buttonBox()->addButton(tr("Refresh name"),
...
void RoomSettingsDialog::load()
{
    ...
    for (const auto& tag: room->connection()->tagNames()) {
        ...
        item->setCheckState(roomTags.contains(tag) ? Qt::Checked
                                                   : Qt::Unchecked);
        item->setToolTip(tag);
...
void RoomSettingsDialog::apply()
{
    ...
    if (version->text() != room->version()) {
        ...
        connectSingleShot(room, &Room::upgradeFailed, this,
                          &Dialog::applyFailed);
    ...
    , invitees(new QListWidget)
{
    ...
    connect(account, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &CreateRoomDialog::accountSwitched);
#else
    connect(account,
...
#endif
    mainFormLayout->insertRow(0, new QLabel(tr("Please fill the fields as "
                                               "desired. None are mandatory")));

    nextInvitee->setEditable(true);
    nextInvitee->setSizeAdjustPolicy(
    ...
    connect(nextInvitee, &NextInvitee::currentTextChanged, this,
...
void CreateRoomDialog::apply()
{
    ...
    auto* job = connection->createRoom(publishRoom->isChecked()
                                           ? Connection::PublishRoom
                                           : Connection::UnpublishRoom,
                                       alias->text(), roomName->text(),
                                       topic->toPlainText(), userIds, "",
                                       version->currentData().toString(), false);

    connect(job, &BaseJob::success, this, &Dialog::accept);
    connect(job, &BaseJob::failure, this,
            [this, job] { applyFailed(job->errorString()); });
}
...
void CreateRoomDialog::accountSwitched()
{
    ...
    //    auto prefix =
    //            savedCurrentText.midRef(savedCurrentText.startsWith('@') ? 1 :
    //            0);
    //    if (prefix.size() >= 3)
    //    {
    ...
    qDebug() << "Completion candidates:" << model->rowCount() << "out of"
             << connection->users().size() << "filtered in" << et;
    //    }
