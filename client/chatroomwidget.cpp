...
{
    {
        ...
        qmlRegisterUncreatableType<RoomMessageEvent>(
            "Quotient", 1, 0, "RoomMessageEvent",
            "RoomMessageEvent is uncreatable");
    }
    ...
    m_timelineWidget->engine()->addImageProvider(QStringLiteral("mtx"),
                                                 m_imageProvider);
    ...
    connect(m_chatEdit, &ChatEdit::cancelledCompletion, this,
            &ChatRoomWidget::typingChanged);
...
void ChatRoomWidget::setRoom(QuaternionRoom* room)
{
    ...
    if (m_currentRoom) {
        ...
        connect(m_currentRoom, &Room::typingChanged, this,
                &ChatRoomWidget::typingChanged);
        connect(m_currentRoom, &Room::readMarkerMoved, this, [this] {
            const auto rm = m_currentRoom->readMarker();
            readMarkerOnScreen = rm != m_currentRoom->timelineEdge()
                                 && std::lower_bound(indicesOnScreen.begin(),
                                                     indicesOnScreen.end(),
                                                     rm->index())
                                        != indicesOnScreen.end();
            reStartShownTimer();
        ...
        connect(m_currentRoom, &Room::encryption, this,
                &ChatRoomWidget::encryptionChanged);
    ...
    m_timelineWidget->rootContext()->setContextProperty(QStringLiteral("room"),
                                                        room);
    typingChanged();
    encryptionChanged();
...
void ChatRoomWidget::typingChanged()
{
    ...
    setHudCaption(
        tr("Currently typing: %1").arg(typingNames.join(QStringLiteral(", "))));
}
...
void ChatRoomWidget::focusInput() { m_chatEdit->setFocus(); }
...
QString ChatRoomWidget::doSendInput()
{
    ...
    if (!attachedFileName.isEmpty()) {
        ...
        auto txnId = m_currentRoom->postFile(
            text.isEmpty() ? QUrl(attachedFileName).fileName() : text,
    ...
    {
        m_currentRoom->postPlainText(
            (argString.isEmpty() ? "" : argString + " ") + "¯\\_(ツ)_/¯");
        return {};
    }
    if (command == "topic") {
    ...
    if (command == "pm" || command == "msg") {
        ...
        if (UserIdRE.match(args.front()).hasMatch()) {
            if (args.back().isEmpty())
                ...
                m_currentRoom->connection()->doInDirectChat(
                    args.front(),
...
void ChatRoomWidget::quote(const QString& htmlText)
{
    ...
    case 0:
        sendString =
            document.toPlainText().replace(QRegularExpression(defaultRegex),
                                           defaultStyle);
        break;
    case 1:
        sendString =
            document.toPlainText().replace(QRegularExpression(regex), style);
        break;
    case 2:
                              ...
                              bool showingDetails)
{
    ...
    if (eventType == "image" || eventType == "file") {
        const auto progressInfo =
            modelIndex.data(MessageEventModel::SpecialMarksRole)
...
void ChatRoomWidget::reStartShownTimer()
{
    ...
    maybeReadTimer.start(
        Quotient::Settings().get<int>("UI/maybe_read_timer", 1000), this);
