...
MainWindow::MainWindow()
{
    ...
    connect(nam, &QNetworkAccessManager::proxyAuthenticationRequired, this,
            &MainWindow::proxyAuthenticationRequired);
    ...
    connect(chatRoomWidget, &ChatRoomWidget::resourceRequested, this,
            &MainWindow::openResource);
    connect(chatRoomWidget, &ChatRoomWidget::joinRequested, this,
            &MainWindow::joinRoom);
    connect(roomListDock, &RoomListDock::roomSelected, this,
            &MainWindow::selectRoom);
    connect(chatRoomWidget, &ChatRoomWidget::showStatusMessage, statusBar(),
            &QStatusBar::showMessage);
    connect(userListDock, &UserListDock::userMentionRequested, chatRoomWidget,
            &ChatRoomWidget::insertMention);
...
ChatRoomWidget* MainWindow::getChatRoomWidget() const { return chatRoomWidget; }
                                               ...
                                               bool defaultValue)
{
    ...
    auto action = parent->addAction(text, [this, settingsKey](bool checked) {
...
void MainWindow::createMenu()
{
    ...
    static const auto quitShortcut = QSysInfo::productType() == "windows"
                                         ...
                                         : QKeySequence::Quit;
    connectionMenu->addAction(QIcon::fromTheme("application-exit"), tr("&Quit"),
                              qApp, &QApplication::quit, quitShortcut);
    ...
    auto dockPanesMenu =
        viewMenu->addMenu(QIcon::fromTheme("labplot-editvlayout"),
                          tr("Dock &panels",
                             "Panels of the dock, not 'to dock the panels'"));
    roomListDock->toggleViewAction()->setStatusTip(
        tr("Show/hide Rooms dock panel"));
    dockPanesMenu->addAction(roomListDock->toggleViewAction());
    userListDock->toggleViewAction()->setStatusTip(
        tr("Show/hide Users dock panel"));
    dockPanesMenu->addAction(userListDock->toggleViewAction());

    viewMenu->addSeparator();

    auto showEventsMenu = viewMenu->addMenu(tr("&Display in timeline"));
    addTimelineOptionCheckbox(showEventsMenu, tr("Normal &join/leave events"),
                              tr("Show join and leave events"),
                              QStringLiteral("show_joinleave"), true);
    addTimelineOptionCheckbox(
        showEventsMenu, tr("&Redacted events"),
        tr("Show redacted events in the timeline as 'Redacted'"
           " instead of hiding them entirely"),
        QStringLiteral("show_redacted"));
    addTimelineOptionCheckbox(showEventsMenu,
                              tr("&No-effect activity",
                                 "A menu item to show/hide meaningless "
                                 "activity such as redacted spam"),
                              tr("Show/hide meaningless activity (join-leave "
                                 "pairs and redacted events between)"),
    ...
    viewMenu->addAction(tr("Edit tags order"), [this] {
        ...
        bool ok;
        const auto newOrder = QInputDialog::getMultiLineText(
            this, tr("Edit tags order"),
    ...
    viewMenu->addAction(
        QIcon::fromTheme("format-text-blockquote"), tr("Edit quote style"),
        [this] {
            ...
            bool ok;
            const auto newType =
                QInputDialog::getItem(this, tr("Edit quote style"),
    ...
    roomSettingsAction = roomMenu->addAction(
    ...
    roomMenu->addAction(
        QIcon::fromTheme("window-close"), tr("&Close current room"),
...

    {
        ...
        const auto curSetting =
            Settings().value("UI/notifications", fullNotif->data().toString());
    ...
    }
    {
        ...
        connect(layoutGroup, &QActionGroup::triggered, this,
                [this](QAction* action) {
                    ...
                    Settings().setValue("UI/timeline_style",
        ...
        const auto curSetting =
            Settings().value("UI/timeline_style",
    ...
    addTimelineOptionCheckbox(settingsMenu,
    ...
    if (job.error() == QKeychain::Error::EntryNotFound) {
        if (!accessToken.isEmpty()) {
            if (QMessageBox::warning(
                ...
                == QMessageBox::Yes) {
                qDebug() << "Migrating the access token from file to the "
                            "keychain for "
                         << account.userId();
                bool removed = false;
                bool saved =
                    saveAccessTokenToKeyChain(account, accessToken, false);
                if (saved) {
                    QFile accountTokenFile { accessTokenFileName(account) };
                    removed = accountTokenFile.remove();
                }
                if (!(saved && removed)) {
                    qDebug() << "Migrating the access token from the file to "
                                "the keychain failed";
                    QMessageBox::warning(
                        this, tr("Couldn't migrate access token"),
                                       ...
                                       const QByteArray& accessToken)
{
    ...
    if (!((fileDir.exists() || fileDir.mkpath("."))
        ...
        QMessageBox::warning(
            this, tr("Couldn't open a file to save access token"),
        ...
        if ((accountTokenFile.setPermissions(QFile::ReadOwner | QFile::WriteOwner)
             && !(accountTokenFile.permissions()
                  & (QFile::ReadGroup | QFile::ReadOther)))
            || QMessageBox::warning(
                   this, tr("Couldn't set access token file permissions"),
        ...
        if (job.error() != QKeychain::Error::NoBackendAvailable
            ...
            if (writeToFile) {
                const auto button = QMessageBox::warning(
                    ...
                    tr("Quaternion couldn't save the access token to the "
                       "keychain."
...
void MainWindow::enableDebug() { chatRoomWidget->enableDebug(); }
...
void MainWindow::addConnection(Connection* c, const QString& deviceName)
{
    ...
    connect(
        c, &Connection::syncError, this,
        [this, c](const QString& message, const QString& details) {
            QMessageBox msgBox(
                QMessageBox::Warning, tr("Sync failed"),
                connections.size() > 1
                    ? tr("The last sync of account %1 has failed with error: "
                         "%2")
    ...
    connect(c, &Connection::requestFailed, this, [this](BaseJob* job) {
        if (job->isBackground())
            return;
        auto message =
            job->error() == BaseJob::UserConsentRequiredError
        ...
        QMessageBox msgBox(QMessageBox::Warning, job->statusCaption(), message,
                           QMessageBox::Close, this);
        msgBox.setTextFormat(Qt::RichText);
        msgBox.setDetailedText(tr("Request URL: %1\nResponse:\n%2")
                                   .arg(job->requestUrl().toDisplayString(),
                                        job->rawDataSample()));
    ...
    connect(c, &Connection::loginError, this,
            [=](const QString& msg) { loginError(c, msg); });
    ...
    accountMenu->addAction(
        QIcon::fromTheme("view-certificate"), tr("Show &access token"), this,
        [=] {
            const QString aToken = c->accessToken();
            auto accountTokenBox = new QMessageBox(
    ...
    connect(c, &Connection::destroyed, connectionMenu,
            [this, menuAction] { connectionMenu->removeAction(menuAction); });
...
void MainWindow::showAboutWindow()
{
    ...
    auto* tabWidget = new QTabWidget();
    {
        ...
        auto* linkLabel = new QLabel(
            "<a "
            "href=\"https://matrix.org/docs/projects/client/quaternion.html\">"
            + tr("Web page") + "</a>");
    ...
    }
    {
        auto* thanksLabel = new QLabel(
            tr("Original project author: %1")
                .arg("<a href='https://github.com/Fxrh'>Felix Rohrbach</a>")
            + "<br/>"
            + tr("Project leader: %1")
                  .arg(
                      "<a href='https://github.com/KitsuneRal'>Kitsune Ral</a>")
            + "<br/><br/>" + tr("Contributors:")
            + "<br/>" + "<a href='https://github.com/quotient-im/Quaternion/graphs/contributors'>"
            + tr("Quaternion contributors @ GitHub") + "</a><br/>"
            + "<a "
              "href='https://github.com/quotient-im/libQuotient/graphs/"
              "contributors'>"
            + tr("libQuotient contributors @ GitHub") + "</a><br/>"
            + "<a "
              "href='https://lokalise.co/contributors/"
              "730769035bbc328c31e863.62506391/'>"
            + tr("Quaternion translators @ Lokalise.co") + "</a><br/>"
            + tr("Special thanks to %1 for all the testing effort")
                  .arg("<a "
                       "href='https://matrix.to/#/"
                       "@nep:pink.packageloss.eu'>nepugia</a>")
            + "<br/><br/>" + tr("Made with:") + "<br/>"
            + "<a href='https://www.qt.io/'>Qt 5</a><br/>"
              "<a "
              "href='https://www.qt.io/"
              "qt-features-libraries-apis-tools-and-ide/#ide'>Qt "
              "Creator</a><br/>"
...
void MainWindow::logout(Connection* c)
{
    ...
    job.setKey(c->userId());
    QEventLoop loop;
    QKeychain::DeletePasswordJob::connect(&job, &QKeychain::Job::finished,
    ...
    if (job.error()) {
        if (job.error() == QKeychain::Error::EntryNotFound)
            qDebug()
                << "Access token is not in the keychain, nothing to delete";
        else {
...
Locator::ResolveResult MainWindow::openLocator(const Locator& l,
...
void MainWindow::openResource(const QString& idOrUri, const QString& action)
{
    ...
    auto l =
        action == "interactive"
            ? id.isEmpty()
                  ? obtainIdentifier(getDefaultConnection(), QFlag(Room | User),
                                     tr("Open room"),
...
void MainWindow::selectRoom(Quotient::Room* r)
{
    ...
    et.start();
    if (currentRoom)
        disconnect(currentRoom, &QuaternionRoom::displaynameChanged, this,
                   nullptr);
                                                     ...
                                                     const QString& prompt)
{
    ...
    QStringList names;
    names.reserve(connections.size());
    int defaultIdx = -1;
    for (auto c: qAsConst(connections)) {
    ...
    bool ok = false;
    const auto choice = QInputDialog::getItem(this, tr("Confirm account"),
                                              prompt, names, defaultIdx, false,
                                              &ok);
                                     ...
                                     const QString& actionName)
{
    ...
    setCompleter(identifier, connections[account->currentIndex()],
                 completionType);
    ...
    connect(identifier, &QLineEdit::textChanged, &dlg, [identifier, okButton] {
        okButton->setDisabled(identifier->text().isEmpty());
    });
...
void MainWindow::joinRoom(const QString& roomIdOrAlias)
{
    ...
    auto roomLocator =
        roomIdOrAlias.isEmpty()
            ? obtainIdentifier(
                defaultConnection, None, tr("Enter room id or alias"),
                tr("Room ID (starting with !)\nor alias (starting with #)"),
                tr("Join"))
            : makeLocator(chooseConnection(defaultConnection,
                                           tr("Confirm account to join %1")
                                               .arg(roomIdOrAlias)),
                          roomIdOrAlias);
    ...
    connect(job, &BaseJob::success, this, [this, roomLocator] {
        statusBar()->showMessage(
            tr("Joined %1 as %2")
                .arg(roomLocator.identifier, roomLocator.account->userId()));
    });
}
...
void MainWindow::showMillisToRecon(Connection* c)
{
    ...
    statusBar()->showMessage(
        tr("Couldn't connect to the server as %1; will retry within %2 seconds")
            .arg(c->userId())
            .arg((c->millisToReconnect() + 999) / 1000)); // Integer ceiling
}
        ...
        if (error.error() == QSslError::NoSslSupport) {
            ...
            if (showMsgBox) {
                QMessageBox msgBox(QMessageBox::Critical, tr("No SSL support"),
                                   error.errorString(), QMessageBox::Close,
                                   this);
...
{
    if (Quotient::SettingsGroup("UI").value("close_to_tray", false).toBool()) {
        hide();
        event->ignore();
    } else {
        event->accept();
